...
class ContextAwareAgent {
    ...
    getAgentId(agentName) {
        // Convert agent name to ID format for Story-Drive agents
        const nameToId = {
            'Plot Doctor': 'plot',
            'Character Coach': 'character',
            'Dialog Director': 'dialog',
            'Dialogue Director': 'dialog',  // Alternative spelling
            'World Builder': 'world',
            'Genre Guide': 'genre',
            'Editor': 'editor',
            'Reader': 'reader',
            'Narrative': 'narrative',
            // Legacy BMAD agent names (for compatibility)
            'Plot Architect': 'plot',
            'Character Psychologist': 'character',
            'Dialogue Coach': 'dialog',
            'Genre Specialist': 'genre',
            'Style Mentor': 'editor',
            'Research Assistant': 'narrative'
        };

        // Try exact match first
        if (nameToId[agentName]) {
            return nameToId[agentName];
        }

        // Try lowercase match
        const normalized = agentName.toLowerCase().replace(/\s+/g, '-');

        // Map common variations to Story-Drive IDs
        if (normalized.includes('plot')) return 'plot';
        if (normalized.includes('character')) return 'character';
        if (normalized.includes('dialog') || normalized.includes('dialogue')) return 'dialog';
        if (normalized.includes('world')) return 'world';
        if (normalized.includes('genre')) return 'genre';
        if (normalized.includes('editor') || normalized.includes('edit')) return 'editor';
        if (normalized.includes('reader') || normalized.includes('audience')) return 'reader';
        if (normalized.includes('narrative') || normalized.includes('narrator')) return 'narrative';
